...
def KL_symm(a, b):
    return np.sum(np.where(a !=0 and b !=0, a * np.log(a / b) + b * np.log(b / a), 0))

def align_ppg_data(org_data,tar_data,s_post,t_post):
    j_data = []
    for i in range(len(s_post)):
        score = []
        for j in range(len(t_post)):
            score.append(KL(s_post[i],t_post[j]))
        j_data.append(np.r_[org_data[i],tar_data[score.index(min(score))]])
    return np.array(j_data)

def align_ppg_sym_data(org_data, tar_data, s_post, t_post):
    ''' a * np.log(a/b) + b * np.log(b / a)'''
    j_data = []
    for i in range(len(s_post)):
        score = []
        for j in range(len(t_post)):
            score.append(KL_symm(s_post[i], t_post[j]))
        j_data.append(np.r_[org_data[i],tar_data[score.index(min(score))]])
    return np.array(j_data)
